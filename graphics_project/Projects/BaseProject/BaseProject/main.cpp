...
void KeyboardFunc(unsigned char c, int x, int y)
{
	...
	case '+':
		...
		window.CameraMode++;
		if(window.CameraMode > window.PEPSI_VIEW) window.CameraMode = window.SATELLITE_VIEW;
		break;

	case '-':
		window.pan_angle = 0.0f;
		window.CameraMode--;
		if(window.CameraMode < window.SATELLITE_VIEW) window.CameraMode = window.PEPSI_VIEW;
		break;
...
void DisplayFunc()
{
	...
	if (window.CameraMode == window.SATELLITE_VIEW) {
	...
	} else if (window.CameraMode == window.PEPSI_VIEW) {
		// PEPSI CAN!
		...
		can.Draw(projection, modelview, eye, window.size, (window.paused ? window.time_last_pause_began : current_time) - window.total_time_paused, window.CameraMode);
